...
def loss(logits, labels, class_weights=None):
  ...
  # height_to_crop = int((data.INPUT_HEIGHT - data.OUTPUT_HEIGHT)  / 2)
  # width_to_crop = int((data.INPUT_WIDTH - data.OUTPUT_WIDTH)  / 2)

  # cropped_labels = tf.slice(
  #     labels, 
  #     begin=[0, height_to_crop, width_to_crop, 0],
  #     size=[-1, data.OUTPUT_HEIGHT, data.OUTPUT_WIDTH, -1])

  # flatten_labels = tf.reshape(cropped_labels, [-1])
